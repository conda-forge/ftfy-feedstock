...
Once the `conda-forge` channel has been enabled, `ftfy` can be installed with `conda`:
...
It is possible to list all of the versions of `ftfy` available on your platform with `conda`:
...
```
mamba search ftfy --channel conda-forge
```
...
mamba repoquery search ftfy --channel conda-forge

# List packages depending on `ftfy`:
mamba repoquery whoneeds ftfy --channel conda-forge

# List dependencies of `ftfy`:
mamba repoquery depends ftfy --channel conda-forge
```
